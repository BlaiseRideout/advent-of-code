...
fn simulate_rocks(
    ...
    static REPEAT_LENGTH: usize = 7000;
    static REPEAT_START_ROCK: usize = 251000;

    let position_in_sequence = num_rocks % REPEAT_LENGTH;

    let mut repeat_start_height = 0;

    for i_rock in 0..=REPEAT_LENGTH {
        if i_rock > REPEAT_START_ROCK && i_rock - REPEAT_START_ROCK == position_in_sequence {
            break;
        }
